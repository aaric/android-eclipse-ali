...
public final class DownloadUtils {
	 ...
	 */
	private static final int DEFAULT_DOWNLOAD_TOTAL_THREAD = 3;

	/**
	 * Default download cache record directory.
	 */
	private static File DEFAULT_DOWNLOAD_CACHE_RECORD_DIRECTORY = Environment
				...
				if (HttpURLConnection.HTTP_OK == code) {
					...
					if (!isToDownload) {
						...
						for (int threadIndex = 1; threadIndex <= threadNumber; threadIndex++) {
							...
							// Download.
							new DownloadThread(resUrl, storagePath, threadIndex, startSize, endSize).start();
	 ...
	 * @return
	 */
	public static boolean download(Context context, String resUrl,
			String storagePath) {
		// Initialize
		File directory = new File(context.getCacheDir(), "downloads");
		if (null != directory) {
			// Create cache download recored directory.
			if (!directory.exists()) {
				directory.mkdirs();
			}
			try {
				DEFAULT_DOWNLOAD_CACHE_RECORD_DIRECTORY = directory;
				doLogOutput(DEFAULT_DOWNLOAD_CACHE_RECORD_DIRECTORY.getAbsolutePath());
			} catch (Exception e) {
				doLogOutput("The directory appointed is not usable...");
			}
		}
	...
	public static class DownloadThread extends Thread {
		...
		public void run() {
			...
			try {
				...
				recordFile = new File(DEFAULT_DOWNLOAD_CACHE_RECORD_DIRECTORY, record);
				...
				if (recordFile.exists() && 0 < recordFile.length()) {
					...
					doLogOutput(threadIndex + ": start-->" + start + ", end-->" + endSize);
					fis.close();
				...
				conn.setRequestProperty("Range", "bytes=" + start + "-" + endSize);
				int code = conn.getResponseCode();
				...
				if (HttpURLConnection.HTTP_PARTIAL == code) {
					...
					while ((len = is.read(buffer)) != -1) {
						RandomAccessFile rafRecord = new RandomAccessFile(recordFile, "rwd");
					...
					// Output.
					doLogOutput("Thread Index " + threadIndex + " has been download...");
					...
					// Trace.
					doLogOutput("Thread Index " + threadIndex + " download failure...");
